...
```bash
pnpm install       # once
pnpm dev           # the game client, on http://localhost:5173
pnpm dev:server    # the world server, in a second terminal
```

`pnpm dev` alone will start the client and connect it to whatever server is
configured in `apps/client/.env` (by default, the local one). Running both
commands gives you the full game on your own machine.

To play against the deployed staging world instead, start the client with:

```bash
VITE_GAME_SERVER_URL=wss://acorn-ash-web-staging.workers.dev pnpm dev
```
